...
/**
 * 通用文件拖拽处理类 支持文件与文件夹的拖拽提取，并自动处理浏览器兼容问题。
 *
 * @example
...
export default class DragDropProcessor {
  ...
  /** 创建拖拽处理事件集合 可直接绑定到 DOM 或 Vue/React 组件事件上 */
  ...
  /** 从 DataTransferItemList 提取文件 支持目录递归读取 */
  private async extractFiles(items: DataTransferItemList): Promise<File[]> {
    const results: File[] = [];
    const tasks: Promise<void>[] = [];

    // 检查浏览器兼容性
    const supportEntry = Boolean(items[0] && (items[0] as any).webkitGetAsEntry);
    if (!supportEntry) {
      // 不支持 webkit API，直接降级
  ...
  /** 递归读取目录下的所有文件 某些浏览器会分批返回文件项，因此需持续读取 */
  ...
  /** 从 FileSystemFileEntry 获取 File 对象 同时尝试设置 webkitRelativePath 以保留目录结构信息 */
