 ...
 * RAF 节流工具类
 *
 * 框架无关的 RAF 节流实现，供核心类使用。 注意：这是纯函数工具，不依赖 Vue，可以在类中使用。
 *
 * @example
 *   ```typescript
 *   // 在类中使用
 ...
 * RAF 节流工具类
 *
 * 使用 requestAnimationFrame 对函数执行进行节流，确保每帧最多执行一次。 适用于需要高频触发但不需要每帧都执行的场景。
 *
 * @template T 要节流的值类型
 */
export class RafThrottle<T = void> {
   ...
   * 节流执行函数
   *
   * 调用此方法会进行 RAF 节流，确保每帧最多执行一次。 如果多次调用，会使用最新的值执行。
   *
