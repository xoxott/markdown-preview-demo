...
import { DragDropOptions, DragState, DropZoneState, FileItem } from '../types/file-explorer';
export function useFileDragDrop(options: DragDropOptions = {}) {
  ...
  /**
   * 全局 dragend 事件处理（推荐在组件中监听）
   * 注意：这个函数需要在组件的 @dragend 事件中调用
   * 或者使用增强版 useFileDragDropEnhanced 自动处理
   */
