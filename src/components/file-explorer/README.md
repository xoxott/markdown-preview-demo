...
- [ ] 批量操作进度显示
- [ ] 文件上传功能（服务器模式）
- [ ] 性能优化（大文件/大量文件）：流式处理、虚拟滚动、Web Workers
...
- **文本文件**: 自动识别语言，使用 Monaco 编辑器编辑
- **图片文件**: 支持常见图片格式预览
- **Markdown**: 支持 Markdown 渲染预览
- **编辑功能**: 双击文件或右键菜单"编辑"打开编辑器
...
- `createFolder(path, name)` - 创建文件夹
- `deleteFile(path)` - 删除文件
- `renameFile(path, newName)` - 重命名
- `copyFile(srcPath, destPath)` - 复制文件
- `moveFile(srcPath, destPath)` - 移动文件
...
const files = await localDataSource.listFiles('/');
```
...
const files = await serverDataSource.listFiles('/');
```
