...
import { Transition, computed, defineComponent, onMounted, onUnmounted, ref, watch } from 'vue';
 ...
 * @property {boolean} [canDrop=true] 是否允许放置文件或文件夹. Default is `true`
 * @property {boolean} [isOver=false] 父组件传入的"正在被拖拽悬停"的外部状态. Default is `false`
 * @property {boolean} [disabled=false] 是否禁用该放置区. Default is `false`
 * @property {boolean} [asFolderZone=false] 是否作为文件夹模式（在文件夹卡片内使用）. Default is `false`
...
export default defineComponent({
  ...
  /**
   * @fires dragEnter (zoneId: string) - 当拖拽进入该区域时触发
   * @fires dragLeave (zoneId: string) - 当拖拽离开该区域时触发
   * @fires drop (zoneId: string) - 当文件被放置到该区域时触发
   */
            ...
            <div class="relative">
              ...
              <div class="flex flex-col items-center justify-center gap-4 text-center">
                ...
                <div class="relative">
                  ...
                  >
                    {canAcceptDrop.value && (
                      <div class="absolute left-1/2 animate-bounce -bottom-8 -translate-x-1/2">
                        <NIcon component={ArrowDownOutline} size={20} class="text-blue-500" />
                      </div>
                    )}
                  </Transition>
                </div>
                ...
                <div class="space-y-1">
                  ...
                  {props.showUploadHint && !props.isOver && !props.loading && (
                    <p class="text-xs text-gray-400 dark:text-gray-500">支持拖拽文件和文件夹</p>
                  )}
                </div>
              ...
              >
                {canAcceptDrop.value && (
                  <div class="pointer-events-none absolute inset-0 rounded-lg bg-blue-500/5 dark:bg-blue-500/10" />
                )}
              </Transition>
              {/* 错误覆盖层 */}
              <Transition
              ...
              >
                {props.isOver && !props.canDrop && (
                  <div class="pointer-events-none absolute inset-0 rounded-lg bg-red-500/5 dark:bg-red-500/10" />
                )}
              </Transition>
            </div>
          )}
