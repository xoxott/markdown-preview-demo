...
export interface UseSearchFormOptions {
  ...
  /**
   * 用户点击「搜索」或回车触发：入参为当前表单浅拷贝快照。
   * 与 `useTable` 组合时通常在此 `updateSearchParams({ page: 1, ... })` 并 `getData()`。
   */
  onSearch?: (values: Record<string, any>) => void;
  /**
   * 表单已恢复默认值后触发（在 restoreValidation 之后）。
   * 与 `useTable` 组合时通常在此 `resetSearchParams()` 并 `getData()`。
   */
  onReset?: () => void;
}

/**
 * 将声明式 searchConfig 转为可写表单 model，并提供搜索 / 重置 / 读写工具。
 * 与 SearchBar、TablePage 配合使用；若页面已用 `useTable` / `useTablePage`，
 * 一般直接使用其 `searchParams` 作为表单 model，无需再套一层本 hook。
 */
