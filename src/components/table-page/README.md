...
- **筛选项**：`NForm` → `NGrid` → `NGi` + `NFormItem`，列数由 `cols` 控制（默认 `1 s:2 m:3 l:5`）。
- **操作区**：`NGi suffix`，占**最后一行剩余列**，重置 → 搜索 → 展开/收起，右对齐。
...
- 默认 `cols: 1 s:2 m:3 l:5`（`SEARCH_GRID_COLS`），`gridResponsive: screen`。
