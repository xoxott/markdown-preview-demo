...
export default defineComponent({
  ...
  setup() {
    ...
    // ==================== Flow 示例 5: 性能测试（1000个节点） ====================
    // 生成1000个节点
    const generatePerformanceNodes = (): FlowNode[] => {
    ...
    // 生成一些连接线（每行连接相邻节点）
    const generatePerformanceEdges = (): FlowEdge[] => {
      ...
      for (let row = 0; row < rows; row++) {
        for (let col = 0; col < cols - 1; col++) {
          ...
          edges.push({
            id: `perf-edge-${sourceIndex}-${targetIndex}`,
    ...
    // 直接生成性能测试数据
    const performanceNodes = ref<FlowNode[]>(generatePerformanceNodes());
    const performanceEdges = ref<FlowEdge[]>(generatePerformanceEdges());
    ...
    return () => (
      <div class="p-4 space-y-4">
        ...
        {/* Flow 示例 5: 性能测试（1000个节点） */}
        <NCard bordered>
          <NH3 class="border-b pb-2 text-lg font-semibold">Flow 示例 5: 性能测试（1000个节点）</NH3>
          <NText class="text-gray-500 mb-4 block">
            测试大量节点的渲染性能，包含1000个节点和975条连接线。建议开启视口裁剪和性能优化配置。
          </NText>
          <NText class="text-sm text-gray-400 mb-4 block">
            节点数: {performanceNodes.value.length}, 连接数: {performanceEdges.value.length}
          </NText>
            ...
            <FlowCanvas
              ...
              config={{
                performance: {
                  ...
                  enableEdgeCanvasRendering: false, // 暂时使用 SVG 渲染
