...
这是一个通用的 HTTP 请求封装库，支持拦截器、错误处理、请求重试、Loading 提示、日志记录、Token 刷新等功能，**不依赖任何 UI 库**，可在应用层配置。
...
```typescript
import { createRequest } from '@ellwood/shared/src/api';
...
```typescript
import { loadingManager, errorMessageManager } from '@ellwood/shared/src/api';
...
```typescript
import { configureTokenRefresh } from '@ellwood/shared/src/api';
...
```typescript
import { retryRequest, shouldRetry } from '@ellwood/shared/src/api';
...
```typescript
import { configureRequestDedupe, requestDedupeManager } from '@ellwood/shared/src/api';
...
```typescript
import { configureRequestCache, requestCacheManager } from '@ellwood/shared/src/api';
...
```typescript
import { configureLogger, loggerManager } from '@ellwood/shared/src/api';
...
// 手动记录日志
import { logRequest, logResponse, logError } from '@ellwood/shared/src/api';
...
// 使用进度跟踪工具
import { createProgressTracker, formatFileSize, formatSpeed } from '@ellwood/shared/src/api';
...
```typescript
// 全局错误处理
import { globalErrorHandlerManager } from '@ellwood/shared/src/api';
...
```typescript
import { circuitBreakerManager, CircuitBreakerState } from '@ellwood/shared/src/api';
...
// 手动管理取消 Token
import { cancelTokenManager, generateRequestId } from '@ellwood/shared/src/api';
...
```typescript
import { loadingManager, showLoading, hideLoading, resetLoading } from '@ellwood/shared/src/api';
...
// 配置 Loading 延迟显示（可选）
import { loadingManager } from '@ellwood/shared/src/api';
