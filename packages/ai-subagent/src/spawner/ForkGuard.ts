 ...
 *
 * 扫描消息历史中的 assistant 消息，检查 content 是否包含 `<fork-boilerplate>` 标签。
 * 如果发现标签，说明当前 AgentLoop 是一个 fork 子代理，不应再 spawn fork。
 *
 * @param messages 消息历史
 ...
 * 计算当前 fork 嵌套深度
 *
 * 深度 = 消息历史中包含 `<fork-boilerplate>` 的 assistant 消息数量。
 * 深度 0 = 在父 AgentLoop 中（无 fork 标签）。
 * 深度 1 = 在一级 fork child 中。
 * 深度 2 = 在二级嵌套 fork child 中（不允许）。
 *
 * @param messages 消息历史
 * @returns fork 嵌套深度
 */
