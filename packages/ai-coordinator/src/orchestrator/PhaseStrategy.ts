    ...
    switch (phase) {
      case 'research': {
        const agents = context.availableAgents.filter(
          a => a.whenToUse.toLowerCase().includes('research') || a.agentType.includes('research')
        );
        return agents.map(a => ({
          phase: 'research',
      ...
      case 'verification': {
        ...
        return agents.map(a => ({
          phase: 'verification',
          agentType: a.agentType,
          prompt: `验证以下实现: ${context.userRequest}\n实现结果: ${context.completedSteps
            .filter(s => s.step.phase === 'implementation')
            .map(s => s.output)
            .join('\n')}`
        }));
      }
