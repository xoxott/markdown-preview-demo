/** G17: HookWorktreeRemovePhase — 工作树移除的 Hook 拦截阶段 */

import type { AgentEvent, LoopPhase, MutableAgentContext } from '@suga/ai-agent-loop';
import type { WorktreeRemoveInput } from '../types/input';
...
export class HookWorktreeRemovePhase implements LoopPhase {
  ...
  async *execute(
    ...
    const worktreeName = ctx.meta.worktreeRemoveName as string | undefined;
    if (worktreeName) {
      const matchingHooks = this.executor.getMatchingHookDefinitions('WorktreeRemove');
      if (matchingHooks.length > 0) {
        const hookContext = this.buildHookContext(ctx);
        const input: WorktreeRemoveInput = {
          hookEventName: 'WorktreeRemove',
          worktreeName,
          worktreePath: (ctx.meta.worktreeRemovePath as string) ?? '',
          branchName: ctx.meta.worktreeRemoveBranch as string
        };
        const aggregated = await this.executor.execute('WorktreeRemove', input, hookContext);
