/** G17: HookWorktreeCreatePhase — 工作树创建的 Hook 拦截阶段 */

import type { AgentEvent, LoopPhase, MutableAgentContext } from '@suga/ai-agent-loop';
import type { WorktreeCreateInput } from '../types/input';
...
export class HookWorktreeCreatePhase implements LoopPhase {
  ...
  async *execute(
    ...
    const worktreeName = ctx.meta.worktreeCreateName as string | undefined;
    if (worktreeName) {
      const matchingHooks = this.executor.getMatchingHookDefinitions('WorktreeCreate');
      if (matchingHooks.length > 0) {
        const hookContext = this.buildHookContext(ctx);
        const input: WorktreeCreateInput = {
          hookEventName: 'WorktreeCreate',
          worktreeName,
          worktreePath: (ctx.meta.worktreeCreatePath as string) ?? '',
          branchName: (ctx.meta.worktreeCreateBranch as string) ?? ''
        };
        const aggregated = await this.executor.execute('WorktreeCreate', input, hookContext);
