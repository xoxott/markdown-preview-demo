/** G17: HookElicitationPhase — 向用户提问的 Hook 拦截阶段 */

import type { AgentEvent, LoopPhase, MutableAgentContext } from '@suga/ai-agent-loop';
import type { ElicitationInput } from '../types/input';
...
export class HookElicitationPhase implements LoopPhase {
  ...
  async *execute(
    ...
    const toolUseId = ctx.meta.elicitationToolUseId as string | undefined;
    if (toolUseId) {
      const matchingHooks = this.executor.getMatchingHookDefinitions('Elicitation');
      if (matchingHooks.length > 0) {
        const hookContext = this.buildHookContext(ctx);
        const input: ElicitationInput = {
          hookEventName: 'Elicitation',
          questions: (ctx.meta.elicitationQuestions as ElicitationInput['questions']) ?? [],
          toolUseId
        };
        const aggregated = await this.executor.execute('Elicitation', input, hookContext);
