/** G17: HookTaskCompletedPhase — 任务完成的 Hook 拦截阶段 */

import type { AgentEvent, LoopPhase, MutableAgentContext } from '@suga/ai-agent-loop';
import type { TaskCompletedInput } from '../types/input';
...
export class HookTaskCompletedPhase implements LoopPhase {
  ...
  async *execute(
    ...
    const taskId = ctx.meta.taskCompletedId as string | undefined;
    if (taskId) {
      const matchingHooks = this.executor.getMatchingHookDefinitions('TaskCompleted');
      if (matchingHooks.length > 0) {
        const hookContext = this.buildHookContext(ctx);
        const input: TaskCompletedInput = {
          hookEventName: 'TaskCompleted',
          taskId,
          subject: (ctx.meta.taskCompletedSubject as string) ?? '',
          owner: ctx.meta.taskCompletedOwner as string
        };
        const aggregated = await this.executor.execute('TaskCompleted', input, hookContext);
