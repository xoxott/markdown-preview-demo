/** G17: HookInstructionsLoadedPhase — 系统指令加载完成的 Hook 拦截阶段 */

import type { AgentEvent, LoopPhase, MutableAgentContext } from '@suga/ai-agent-loop';
import type { InstructionsLoadedInput } from '../types/input';
...
export class HookInstructionsLoadedPhase implements LoopPhase {
  ...
  async *execute(
    ...
    const sessionId = ctx.meta.instructionsLoadedSessionId as string | undefined;
    if (sessionId) {
      const matchingHooks = this.executor.getMatchingHookDefinitions('InstructionsLoaded');
      if (matchingHooks.length > 0) {
        const hookContext = this.buildHookContext(ctx);
        const input: InstructionsLoadedInput = {
          hookEventName: 'InstructionsLoaded',
          sessionId,
          systemPrompt: ctx.meta.instructionsLoadedSystemPrompt as string,
          memoryContent: ctx.meta.instructionsLoadedMemoryContent as string
        };
        const aggregated = await this.executor.execute('InstructionsLoaded', input, hookContext);
