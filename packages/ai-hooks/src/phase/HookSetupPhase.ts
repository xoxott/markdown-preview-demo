...
export class HookSetupPhase implements LoopPhase {
  ...
  async *execute(
    ...
    const sessionId = ctx.meta.setupSessionId as string | undefined;
    if (sessionId) {
      const matchingHooks = this.executor.getMatchingHookDefinitions('Setup');
      if (matchingHooks.length > 0) {
        const hookContext = this.buildHookContext(ctx);
        const input: SetupInput = {
          hookEventName: 'Setup',
          sessionId,
          projectPath: ctx.meta.setupProjectPath as string,
          model: ctx.meta.setupModel as string
        };
        const aggregated = await this.executor.execute('Setup', input, hookContext);
