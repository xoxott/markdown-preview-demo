...
/** 创建简单 hook handler */
function createHandler(name: string, outcome: HookResult['outcome'] = 'success'): HookDefinition['handler'] {
...
describe('HookRegistry', () => {
  describe('register', () => {
    ...
    it('拒绝重复名称', () => {
      ...
      expect(() => registry.register({ name: 'dup', event: 'PreToolUse', handler: createHandler('dup2') }))
        .toThrow('已存在');
    });
  ...
  describe('getMatchingHooks', () => {
    ...
    it('有 matcher 精确匹配', () => {
      const registry = new HookRegistry();
      registry.register({ name: 'bash-hook', event: 'PreToolUse', matcher: 'Bash', handler: createHandler('bash') });
      registry.register({ name: 'write-hook', event: 'PreToolUse', matcher: 'Write', handler: createHandler('write') });
      expect(registry.getMatchingHooks('PreToolUse', 'Bash')).toHaveLength(1);
    ...
    it('glob 模式匹配', () => {
      const registry = new HookRegistry();
      registry.register({ name: 'bash-glob', event: 'PreToolUse', matcher: 'Bash*', handler: createHandler('bash-glob') });
    ...
    it('不匹配时返回空数组', () => {
      const registry = new HookRegistry();
      registry.register({ name: 'bash-only', event: 'PreToolUse', matcher: 'Bash', handler: createHandler('bash') });
      expect(registry.getMatchingHooks('PreToolUse', 'Write')).toHaveLength(0);
    });

    it('无 matchQuery 返回全部', () => {
      const registry = new HookRegistry();
      registry.register({ name: 'h1', event: 'PreToolUse', matcher: 'Bash', handler: createHandler('1') });
      registry.register({ name: 'h2', event: 'PreToolUse', matcher: 'Write', handler: createHandler('2') });
