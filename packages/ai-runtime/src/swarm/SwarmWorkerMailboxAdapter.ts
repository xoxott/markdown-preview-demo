/**
 * SwarmWorkerMailboxAdapter — 将 ai-coordinator 的 PermissionBubbleHandler 适配为 ai-tool-core 的
 * SwarmWorkerMailboxOps 接口
 *
 * ai-tool-core 不依赖 ai-coordinator（避免循环依赖）， 所以定义独立的 SwarmWorkerMailboxOps 接口。 ai-runtime
 * 同时依赖两者，负责桥接适配。
 *
 * 映射关系（近乎 1:1）:
 *
...
import { pollPermissionBubbleResponse, sendPermissionBubble } from '@suga/ai-coordinator';
 ...
 *
 * 实现 SwarmWorkerMailboxOps 接口，将权限请求投递到 ai-coordinator 的 Mailbox 并轮询 Leader 响应。
 */
...
function mapToBubbleSuggestions(
  suggestion: SwarmPermissionRequest['classifierSuggestion']
): PermissionBubbleSuggestion[] {
