 ...
 *
 * 在 CallModelPhase 之前执行压缩管线，将压缩后的消息写入 ctx.meta.compressedMessages。 CallModelPhase 需感知此 meta
 * 字段来决定使用哪组消息调用 LLM。
 */
export class CompressPhase implements LoopPhase {
  ...
  async *execute(
    ...
    // 检查 collapseInProgress 标记（与 ContextCollapse 互斥时跳过 AutoCompact）
    const skipAutoCompact = ctx.meta.collapseInProgress === true;

    // 执行压缩管线
    const result = await this.pipeline.compress(ctx.state.messages, skipAutoCompact);
