...
(function () {
  ...
  let shimMode = hasDocument ? Boolean(esmsInitOptions.shimMode) : true;
  ...
  const edge = !navigator.userAgentData && Boolean(navigator.userAgent.match(/Edge\/\d+\.\d+/));
  ...
  function resolveAndComposeImportMap(json, baseUrl, parentMap) {
    const outMap = { imports: { ...parentMap.imports }, scopes: { ...parentMap.scopes } };
  ...
  const dynamicImportCheck =
    ...
    new Promise(resolve => {
      ...
      s.addEventListener('load', () => {
        if (!(supportsDynamicImport = Boolean((dynamicImport = self._d)))) {
          let err;
  ...
  const featureDetectionPromise = Promise.resolve(dynamicImportCheck).then(() => {
    ...
    return new Promise(resolve => {
      ...
      const importMapTest = `<script nonce=${nonce || ''}>b=(s,type='text/javascript')=>URL.createObjectURL(new Blob([s],{type}));document.head.appendChild(Object.assign(document.createElement('script'),{type:'importmap',nonce:"${nonce}",innerText:\`{"imports":{"x":"\${b('')}"}}\`}));Promise.all([${
        supportsImportMaps ? 'true,true' : `'x',b('${importMetaCheck}')`
      }, ${cssModulesEnabled ? `b('${cssModulesCheck}'.replace('x',b('','text/css')))` : 'false'}, ${
        jsonModulesEnabled ? `b('${jsonModulesCheck}'.replace('x',b('{}','text/json')))` : 'false'
  ...
  function parse(E, g = '@') {
    ...
    const I = E.length + 1;
    const o = (C.__heap_base.value || C.__heap_base) + 4 * I - C.memory.buffer.byteLength;
    ...
    for (; C.ri(); ) {
      ...
      const o = C.se();
      let D;
      C.ip() && (D = w(E.slice(g === -1 ? A - 1 : A, g === -1 ? Q + 1 : Q))),
        J.push({ n: D, s: A, e: Q, ss: I, se: o, d: g, a: B });
    }
    for (; C.re(); ) {
      const A = C.es();
      const Q = C.ee();
  ...
  const resolve = resolveHook
    ? async (id, parentUrl) => {
        ...
        return result
          ? { r: result, b: !resolveIfNotPlainOrUrl(id, parentUrl) && !isURL(id) }
  ...
  function throwUnresolved(id, parentUrl) {
    throw new Error(`Unable to resolve specifier '${id}'${fromParent(parentUrl)}`);
  }
  ...
  async function doFetch(url, fetchOpts, parent) {
    ...
    if (!res.ok) throw new Error(`${res.status} ${res.statusText} ${res.url}${fromParent(parent)}`);
    return res;
  }
  ...
  async function fetchModule(url, fetchOpts, parent) {
    ...
    if (jsContentType.test(contentType)) return { r: res.url, s: await res.text(), t: 'js' };
